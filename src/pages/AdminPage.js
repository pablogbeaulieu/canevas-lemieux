...
function AdminPage() {
  ...
  const fetchUsers = async () => {
    const { data, error } = await supabase.from("users").select("id, email, isApproved, role, last_active");
  ...
  // ------------------------------------------------------------
  // ✅ Backup manuel (NOUVE_ATTACHÉ)
  // ------------------------------------------------------------
  const runManualBackup = async () => {
    const ok = window.confirm(
      `Créer un backup maintenant (secteur: ${adminSector}) ?\n\nRecommandé avant une grosse série de modifications.`
    );
    if (!ok) return;
    ...
    try {
      ...
      // invoke => ajoute le JWT automatiquement (verify_jwt: true)
      const { data, error } = await supabase.functions.invoke("backup-canevas-admin", { body: {} });

      if (error) {
        console.error("backup invoke error:", error);
        alert(`❌ Backup échoué.\n${error.message}`);
        return;
      }

      const file = data?.file || "backup créé";
      setBackupMsg(`✅ Backup créé : ${file}`);
  ...
  // ------------------------------------------------------------
  // ✅ Undo dernière action (NOUVEAU)
  // ------------------------------------------------------------
  const undoLastCanevasAction = async () => {
    try {
      setIsUndoing(true);
      setUndoMsg("");

      const { data: prev, error: prevErr } = await supabase.rpc("preview_last_canevas_history_action", {
        p_sector: adminSector,
      });

      if (prevErr) {
        alert(`❌ Preview impossible.\n${prevErr.message}`);
        return;
      }
      if (!prev?.ok) {
        alert(prev?.message || "Aucune action à annuler.");
        return;
      }

      const ok = window.confirm(`${prev.message}\n\nConfirmer l’annulation ?`);
      if (!ok) return;

      const { data, error } = await supabase.rpc("undo_last_canevas_history_action", {
        p_sector: adminSector,
      });

      if (error) {
        alert(`❌ Annulation échouée.\n${error.message}`);
        return;
      }
      if (!data?.ok) {
        alert(data?.message || "Aucune action à annuler.");
        return;
      }

      await fetchCategories();
      if (selectedCategory) await fetchSubCategories();
      if (selectedCategory && selectedSubCategory) await fetchCanevas();

      setUndoMsg("✅ Dernière action annulée.");
      setTimeout(() => setUndoMsg(""), 2600);
    } finally {
      setIsUndoing(false);
    }
  };
  ...
  const deleteCategory = async (cat) => {
    const confirmDelete = window.confirm(`⚠️ Supprimer la catégorie "${cat}" dans le secteur "${adminSector}" ?`);
    ...
    try {
      ...
      if (realCount === 0) {
        const { error } = await supabase.from("canevas").delete().eq("sector", adminSector).eq("category", cat);
  ...
  const deleteCanevas = async (id) => {
    ...
    const { error } = await supabase.from("canevas").delete().eq("sector", adminSector).eq("id", id);
      ...
      <div className="mb-6 rounded-xl bg-gradient-to-r from-blue-700 to-blue-900 text-white p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-white/80 text-sm mt-1">Gestion des accès, canevas, suggestions et nouveautés.</p>
          </div>
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Approbation, rôles et suppression des comptes.</p>
              </div>
              <span className="text-xs bg-white border px-3 py-1 rounded-full text-gray-700">{users.length} utilisateurs</span>
            </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              ...
                              title={text}
                            />
                            <span className="text-xs text-gray-500">{user.role === "admin" ? "Admin" : "Utilisateur"}</span>
                          </div>
            ...
            <div className="mt-6">
              <h3 className="text-lg font-semibold mb-3 border-b pb-2 text-gray-900">Demandes de réinitialisation de mot de passe</h3>
                  ...
                  {resetRequests.map((req) => (
                    <li key={req.id} className="py-3 px-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-600">Reçue le {new Date(req.requested_at).toLocaleString("fr-CA")}</p>
                      </div>
                      <button
        ...
        {showCanevasSection && (
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              ...
              <div className="flex flex-wrap items-center gap-2 p-2 bg-white border rounded-xl">
                ...
                {/* ✅ Bouton Backup (nouveau) */}
                <motion.button
                  type="button"
                  onClick={runManualBackup}
                  disabled={isBackingUp}
                  whileHover={!isBackingUp ? { scale: 1.02 } : {}}
                  whileTap={!isBackingUp ? { scale: 0.98 } : {}}
                  className="text-xs px-3 py-2 rounded-lg border border-blue-200 bg-white hover:bg-blue-50 font-semibold text-blue-900 disabled:opacity-60"
                  title="Créer un backup maintenant (recommandé avant une série de modifications)"
                >
                  {isBackingUp ? "Backup..." : "💾 Backup"}
                </motion.button>

                {/* ✅ Bouton Undo discret + animé */}
                <motion.button
                  type="button"
                  onClick={undoLastCanevasAction}
                  disabled={isUndoing}
                  whileHover={!isUndoing ? { scale: 1.02 } : {}}
                  whileTap={!isUndoing ? { scale: 0.98 } : {}}
                  className="text-xs px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 font-semibold text-gray-700 disabled:opacity-60"
                  title="Annuler la dernière action sur un canevas (ajout / modification / suppression)"
                >
                  {isUndoing ? "Annulation..." : "↩ Annuler"}
                </motion.button>

                {/* Mini feedbacks (discrets) */}
                <AnimatePresence>
                  {backupMsg && (
                    <motion.span
                      key="backup-msg"
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -2 }}
                      className="text-xs bg-blue-50 border border-blue-200 text-blue-900 px-3 py-2 rounded-lg font-semibold"
                    >
                      {backupMsg}
                    </motion.span>
                  )}
                ...
                <AnimatePresence>
                  {undoMsg && (
                    <motion.span
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -2 }}
                      className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-800 px-3 py-2 rounded-lg font-semibold"
                    >
                      {undoMsg}
                    </motion.span>
                  )}
        ...
        {showSuggestionsSection && (
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <span className="text-xs bg-white border px-3 py-1 rounded-full text-gray-700">{suggestions.length} suggestions</span>
            </div>
              ...
              <ul className="space-y-3">
                {suggestions.map((sugg) => (
                  <li key={sugg.id} className="bg-white border rounded-xl p-4">
                    ...
                    <div className="text-xs text-gray-500 mb-3">Reçue le {new Date(sugg.created_at).toLocaleString("fr-CA")}</div>
                    <button
        ...
        {showNewsSection && (
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <span className="text-xs bg-white border px-3 py-1 rounded-full text-gray-700">{newsList.length} items</span>
            </div>
                ...
                {newsList.map((news) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-2">Publié le {new Date(news.created_at).toLocaleString("fr-CA")}</p>
                    </div>
